...
const ArticleSummariesSchema = z.object({
  ...
  details: z.string().describe('Supporting details and evidence that complement the overview in less than 500 characters'),
});
...
`;
  ...
  try {
    const result = await model.withStructuredOutput(ArticlesArraySchema).invoke([new HumanMessage(prompt)]);
 ...
 */
export async function extractWebArticleFullText(htmlContent, sourceUrl) {
  // Check if API key is available
  if (!process.env.OPENAI_API_KEY) {
    throw new Error('OPENAI_API_KEY environment variable is required. Please set it in your .env file or environment.');
  }
