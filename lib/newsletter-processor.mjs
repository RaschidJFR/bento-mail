...
async function processArticle(article, options = {}) {
  ...
  try {
    ...
    const enrichedArticle = {
      ...
      oneliner: webArticle.oneliner || article.header || '',
      url: article.url || '',
...
async function processArticlesInParallel(articles, options = {}) {
  ...
  for (let i = 0; i < articlesWithUrls.length; i += concurrency) {
    ...
    console.log(`Processing batch ${Math.floor(i / concurrency) + 1}/${Math.ceil(articlesWithUrls.length / concurrency)} (${batch.length} articles)`);
 ...
 * @param {string} newsletterHtml - HTML content of the newsletter
 * @param {Object} options - Processing options
 * @returns {Promise<Array>} Array of extracted and processed articles
 */
export async function processNewsletterHtml(newsletterHtml, options = {}) {
  const { concurrency = 3, timeout = 15000, delay = 500, retries = 2 } = options;
