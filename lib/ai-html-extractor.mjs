...
const newsletterModel = new ChatOpenAI({
  modelName: 'gpt-5-mini',
  apiKey: process.env.OPENAI_API_KEY,
});

const articleModel = new ChatOpenAI({
  modelName: 'gpt-5-mini',
  apiKey: process.env.OPENAI_API_KEY,
});
...
const BasicArticleSchema = z.object({
  ...
  summary: z.string().optional().describe('A summary of the article of around 300 characters'),
...
`;
  ...
  try {
    const result = await newsletterModel.withStructuredOutput(ArticlesArraySchema).invoke([new HumanMessage(prompt)]);
...
/**
 * Extracts a single article from HTML content using GPT
 * @param {string} htmlContent - The HTML content of a single article
 * @param {string} [sourceUrl] - Source URL for the article
 * @returns {Promise<Object>} Single article object
 */
export async function extractWebArticle(htmlContent, sourceUrl) {
  // Check if API key is available
  if (!process.env.OPENAI_API_KEY) {
    throw new Error('OPENAI_API_KEY environment variable is required. Please set it in your .env file or environment.');
  }
