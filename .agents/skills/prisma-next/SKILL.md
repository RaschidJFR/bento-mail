...
- **Contract**: `prisma/contract.ts` (TypeScript authoring) — the user's data models. Edit this to add or change models.
...
- **Database client**: `prisma/db.ts` — `import { db } from './prisma/db'`. This is the entry point for all queries.
- **Generated files** (do not edit by hand):
  - `prisma/contract.json` — compiled contract, used at runtime
  - `prisma/contract.d.ts` — TypeScript types for the contract, used for autocomplete and type checking

## Commands

- `npx prisma-next contract emit` — regenerate `contract.json` and `contract.d.ts` after changing the contract
- `npx prisma-next db init` — bootstrap a database to match the contract (creates collections, indexes, constraints). Additive only — won't drop existing structures.
- `npx prisma-next db update` — update the database to match the current contract. Prompts for confirmation on destructive changes. Use `--dry-run` to preview.
- `npx prisma-next migration plan` — create a new migration from contract changes (offline, no database needed). Use `--name <slug>` to name it.
- `npx prisma-next migration apply` — apply pending migrations to the database
- `npx prisma-next migration status` — show which migrations are applied and which are pending
- `npx prisma-next migration show <name>` — show details of a specific migration
...
- **Always emit after contract changes.** When you modify `prisma/contract.ts`, run `npx prisma-next contract emit` before writing any code that depends on the new or changed models.
...
- **Root accessors are emitter-driven.** Use the lowercased plural collection name (e.g. `db.orm.users`, `db.orm.posts`) — not the PascalCase model name. Re-run `npx prisma-next contract emit` if a new model's accessor isn't appearing on `db.orm`.
...
1. Edit `prisma/contract.ts`
2. Run `npx prisma-next contract emit`
...
1. Run `npx prisma-next db init`

**User wants to update the database after changing the contract:**
1. Quick path: `npx prisma-next db update` — compares the database to the contract and applies changes directly
2. Migration path (for production workflows):
   - `npx prisma-next migration plan --name describe-the-change` — creates a migration
   - `npx prisma-next migration apply` — applies pending migrations

**User wants to check what migrations need to be applied:**
1. Run `npx prisma-next migration status`
