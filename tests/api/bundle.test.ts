...
import { Bundle, IUser, User } from '@lib/models';
...
describe('/api/bundle', () => {
  describe('GET', () => {
    ...
    it('can get a bundle for a user by their aliasEmail', async () => {
      await User.where({ _id: user._id }).update({ aliasEmail: 'alias@bar.com' });
      vi.mocked(auth.api.getSession).mockResolvedValueOnce({ user: { email: user.email } } as any);
      const bundle = await Bundle.create({
        user: user._id,
        newsletters: ['nid'],
      ...
      expect(res.status).toBe(200);
      const { result } = await res.json();
      expect(result.user).toStrictEqual(String(bundle.user));
    });
  });
  ...
  describe('POST', () => {
    ...
    it('receives a newsletter and creates a new bundle if none exists', async () => {
      ...
      expect(bundle!.user).toStrictEqual(String(user._id));
    });
    ...
    it('can create a bundle for a user with their aliasEmail', async () => {
      const aliasUser = await User.create({
        email: 'email@bar.com',
        aliasEmail: 'alias@bar.com',
        image: null,
        name: null,
      });
